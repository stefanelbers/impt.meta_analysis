...
#create time_series datafile of pain intensity
ts <- select(dat, author, year, cohort_id, cohort_name, pintens_pre_m, pintens_pre_sd, pintens_post_m, pintens_post_sd,
  ...
  subset(measurements_pain_intensity == "Yes") %>%
  rename(time = pintens_fu1_t,
         instrument_name = pintens_name_measurement_instrument)

dat_clean <- dat %>%
  filter(!is.na(author) & !author == "test" & !author == 0) %>%
  subset(dat$measurements_pain_intensity == "Yes") 
...
ts_long$max_scale <- dplyr::recode(ts_long$max_scale, 
                            "VAS" = 100,
                            "VAS (0-100)" = 100,
                            "PRI" = 78,
                            "MPI: pain severity" = 7,

...
require("knitr")
opts_knit$set(root.dir = "~/impt.meta_analysis/deployment/") #set root for Knitr
```
...
source("R/questionnaires_dataframe.R", local = knitr::knit_global())
```

```{r}
source("R/shiny_fp_prep_md.R", local = knitr::knit_global())
```
...
### flow chart
```{r}
prisma(31894, 90, 17955, 17955, 17588, 367, 304, 63, 55)
```
...
Row {data-height=100}
-------------------------------------
Interactive timeseries with scores on a standardized scale that represents the scoring percentage on each respective measurement instrument. 

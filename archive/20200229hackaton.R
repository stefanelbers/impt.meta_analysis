...
## obtain dataframe to facilitate updating the excel sheet ##
url <- 'https://docs.google.com/spreadsheets/d/1DE08OpdFC2eH8VwAV-xLmsFByGh2VvL0DRDjPLhyo9U/edit?usp=sharing'
dat <- gsheet2tbl(url)

fp_dat <- select(dat, author, year, cohort_id, pintens_pre_m, pintens_pre_sd, pintens_post_m, pintens_post_sd,
...
#create a tibble that includes all column names

datcolnames <- colnames(dat)

colnames <- as.tibble(datcolnames)

# filter specific cohort to create a data extraction form #

hafenbrack_data <- as.data.frame(t(dplyr::filter(dat, author == "Hafenbrack")))
library(formattable)
formattable(hafenbrack_data)                      
...
## change long to broad data format and vice versa: Mees Stefan HUckaton ##
# load packages
library(readxl)
